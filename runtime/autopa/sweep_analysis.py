...
def _bd_segment_metrics(force_t, force_y, k, seg_idx, t_start, t_rise, t_fall,
                       ...
                       min_samples=20, min_sample_rate_hz=40.0):
    ...
    if seg_rate_hz < min_sample_rate_hz:
        reasons.append(
            f"low sample rate {seg_rate_hz:.1f}Hz "
            f"(expected >={min_sample_rate_hz:.1f}Hz)")
        excluded = True
    ...
    for seg_idx, k in enumerate(k_values):
        ...
        for c in range(n_pairs):
            ...
            bd_segs.append(_bd_segment_metrics(
                ...
                min_sample_rate_hz=float(min_segment_rate_hz)))
